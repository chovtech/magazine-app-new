...
export default function ContactUsScreen({ navigation }) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (updated?.data?.contact) {
        setContactContent(cleanHTML(updated.data.contact));
      } else {
        const stored = await getSettings();
        setContactContent(cleanHTML(stored?.data?.contact) || "<p>No Contact Info available.</p>");
      }
      setLoading(false);
    }
    loadData();
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {contactContent ? (
          ...
          <Text style={styles.text}>No Contact Info available.</Text>
        )}
      </ScrollView>
    </View>
  );
